...
namespace BlazorApp.Services
{
    public class HttpService : IHttpService
    {
        ...
        private async Task<T> SendRequest<T>(HttpRequestMessage request)
        {
            ...
            var employee = await _localStorageService.GetItem<Employee>("employee");
            var isApiUrl = !request.RequestUri.IsAbsoluteUri;
            if (employee != null && isApiUrl)
            {
                request.Headers.Authorization = new AuthenticationHeaderValue("Basic", employee.AuthData);
            }
